...
from utils.neural_network import NeuralNetworkModel, load_and_prepare_data, train_and_evaluate_model
...
class Rescuer(AbstAgent):
    def __init__(self, env, config_file, nb_of_explorers=1,clusters=[]):
        ...
        self.nn_model = None
        self.ga = GeneticAlgorithm(population_size=50, mutation_rate=0.01, crossover_rate=0.8, elitism_rate=0.1)
        self.RESC_TIME = self.COST_FIRST_AID
    ...
    def save_cluster_csv(self, cluster, cluster_id):
        filename = f"C:/Users/jrgou/Downloads/VictimSim2/ex03_mas_rescuers/mas/clusters/cluster{cluster_id}.txt"
    ...
    def save_sequence_csv(self, sequence, sequence_id):
        filename = f"C:/Users/jrgou/Downloads/VictimSim2/ex03_mas_rescuers/mas/clusters/seq{sequence_id}.txt"
        with open(filename, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
    ...
    def train_neural_network(self):
        ...
        X, y = load_and_prepare_data('C:/Users/jrgou/Downloads/VictimSim2/ex03_mas_rescuers/mas/datasets/data_300v_90x90/env_vital_signals.txt')
        
        # Criar e treinar o modelo
        self.nn_model = NeuralNetworkModel()
        self.nn_model = train_and_evaluate_model(self.nn_model, X, y)
    ...
    def predict_severity_and_class(self):
        ...
        for vic_id, values in self.victims.items():
            vital_signs = np.array([values[1][3:6]])  # qPA, pulso, frequência respiratória
            severity_value = self.nn_model.predict(vital_signs)[0]
            
            # Classificar com base no valor de gravidade
            if severity_value < 25:
                severity_class = 1  # crítico
            elif severity_value < 50:
                severity_class = 2  # instável
            elif severity_value < 75:
    ...
    def sequencing(self):
        ...
        for cluster in self.clusters:
            victims = list(cluster.keys())
            victims_data = {vic_id: {'position': cluster[vic_id][0], 'gravity': cluster[vic_id][1][6]} for vic_id in victims}
            
            start_pos = (0, 0)  # posição inicial (base)
            end_pos = (0, 0)    # posição final (base)
            
            if not victims:
                continue
            
            best_sequence = self.ga.run(victims, victims_data, start_pos, end_pos, generations=100)
            
            # Criar um novo dicionário ordenado com base na sequência otimizada
            optimized_sequence = {vic_id: cluster[vic_id] for vic_id in best_sequence}
    ...
    def planner(self):
        if not self.sequences:
            print(f"{self.NAME}: No sequences to plan")
            return

        sequence = self.sequences[0]
        start = (0, 0)  # sempre começando na base
        self.plan = []
        self.plan_rtime = self.TLIM

        print(f"{self.NAME}: Planning rescue sequence")
        for vic_id in sequence:
            goal = sequence[vic_id][0]
            print(f"{self.NAME}: Planning path to victim {vic_id} at {goal}")
            path = a_star(start, goal, self.map)
            if path:
                self.plan.extend([(x2-x1, y2-y1) for (x1, y1), (x2, y2) in zip(path, path[1:])])
                start = goal
            else:
                print(f"{self.NAME}: Não foi possível encontrar um caminho para a vítima {vic_id}")

        # Planejar o retorno à base
        goal = (0, 0)
        print(f"{self.NAME}: Planning return path to base")
        path = a_star(start, goal, self.map)
        if path:
            self.plan.extend([(x2-x1, y2-y1) for (x1, y1), (x2, y2) in zip(path, path[1:])])
        else:
            print(f"{self.NAME}: Não foi possível encontrar um caminho de volta à base")

        # Calcular o tempo total do plano
        self.plan_rtime = self.TLIM - (len(self.plan) * self.COST_LINE + self.RESC_TIME * len(sequence))
        print(f"{self.NAME}: Planejamento concluído. Total de ações: {len(self.plan)}")
