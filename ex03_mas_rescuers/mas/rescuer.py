...
class Rescuer(AbstAgent):
    def __init__(self, env, config_file, nb_of_explorers=1, clusters=[]):
        ...
        self.ga = GeneticAlgorithm(
            population_size=50, mutation_rate=0.01, crossover_rate=0.8, elitism_rate=0.1
        )
    ...
    def save_cluster_csv(self, cluster, cluster_id):
        filename = os.path.join(
            os.path.dirname(__file__), "clusters", f"cluster{cluster_id}.txt"
        )
    ...
    def save_sequence_csv(self, sequence, sequence_id):
        filename = os.path.join(
            os.path.dirname(__file__), "clusters", f"seq{sequence_id}.txt"
        )
        with open(filename, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
    ...
    def cluster_victims(self):
        ...
        print(
            f"{self.NAME} ({lower_xlim}, {lower_ylim}) - ({upper_xlim}, {upper_ylim})"
        )
        ...
        for (
            key,
            values,
        ) in self.victims.items():  # values are pairs: ((x,y), [<vital signals list>])
            x, y = values[0]
            if x <= mid_x:
                if y <= mid_y:
    ...
    def predict_severity_and_class(self):
        ...
        for vic_id, values in self.victims.items():
            vital_signs = np.array(
                [values[1][3:6]]
            )  # qPA, pulso, frequência respiratória
    ...
    def sequencing(self):
        ...
        for cluster in self.clusters:
            ...
            victims_data = {
                vic_id: {
                    "position": cluster[vic_id][0],
                    "gravity": cluster[vic_id][1][6],
                }
                for vic_id in victims
            }
            ...
            best_sequence = self.ga.run(
                victims, victims_data, start_pos, end_pos, generations=100
            )
    ...
    def planner(self):
        ...
        for vic_id in sequence:
            ...
            if path:
                self.plan.extend(
                    [(x2 - x1, y2 - y1) for (x1, y1), (x2, y2) in zip(path, path[1:])]
                )
                start = goal
            else:
                print(
                    f"{self.NAME}: Não foi possível encontrar um caminho para a vítima {vic_id}"
                )
        ...
        self.plan_rtime = self.TLIM - (
            len(self.plan) * self.COST_LINE + self.RESC_TIME * len(sequence)
        )
